...
async def create_vehicle(vehicle_data: dict):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post(f"{API_URI}/vehicles/", json=vehicle_data)
...
async def list_vehicles(skip: int = 0, limit: int = 100):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{API_URI}/vehicles/", params={"skip": skip, "limit": limit})
        return resp.json()
...
async def get_vehicle(vehicle_id: int):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"{API_URI}/vehicles/{vehicle_id}")
...
async def update_vehicle(vehicle_id: int, vehicle_data: dict):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.patch(f"{API_URI}/vehicles/{vehicle_id}", json=vehicle_data)
        if resp.status_code == 404:
            return {"error": "Vehicle not found"}
        return resp.json()
...
async def delete_vehicle(vehicle_id: int):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.delete(f"{API_URI}/vehicles/{vehicle_id}")
        if resp.status_code == 404:
            return {"error": "Vehicle not found"}
